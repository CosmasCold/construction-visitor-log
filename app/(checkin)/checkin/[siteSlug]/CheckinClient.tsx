            ...
            {questions.map((q, idx) => (
              <label key={idx} className="flex items-center gap-2 text-sm text-slate-200 mb-2">
                <input
                  ...
                  onChange={(e) => setAnswers((prev) => ({ ...prev, [q]: e.target.checked }))}
                  className="h-4 w-4 rounded border-slate-600 bg-white/10 text-sky-500 focus:ring-sky-500/50"
                />
                {q}
              </label>
              ...
              {photoUrl ? (
                <>
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setPhotoUrl(null)} className="absolute -top-1 -right-1 bg-rose-500 text-white rounded-full w-5 h-5 text-xs leading-none">x</button>
                  </div>
                  <button type="button" onClick={() => printBadgeForVisitor(fullName || "Visitor", company || "", selectedHostId ? hosts.find(h => h.id === selectedHostId)?.name : hostName, photoUrl)} className="flex items-center gap-2 bg-white/10 border border-white/10 rounded-xl px-4 py-2 text-sm text-white hover:bg-white/20 transition-colors">
                ...
                <button type="button" onClick={async () => { const dataUrl = await capturePhoto(); if (dataUrl) await uploadPhoto(dataUrl); }} disabled={uploading} className="flex items-center gap-2 bg-white/10 border border-white/10 rounded-xl px-4 py-2 text-sm text-white hover:bg-white/20 transition-colors disabled:opacity-50">
