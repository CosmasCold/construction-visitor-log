...
export default function RoiCalculatorPage() {
  ...
  const [timePerVisitor, setTimePerVisitor] = useState("2"); // minutes
  const [laborCostPerHour, setLaborCostPerHour] = useState("25"); // USD
  ...
  const dailyLaborCost = dailyTimeHours * parseFloat(laborCostPerHour || "0");
  const annualLaborCost = dailyLaborCost * 260; // working days
  const auditCost =
  ...
  const siteSafeCost = 49 * 12; // $588/year
        ...
        {showResults && (
          <div className="bg-white/[0.06] backdrop-blur-md rounded-2xl border border-white/10 shadow-card-raised p-6 sm:p-8 space-y-6">
            ...
            <div className="bg-emerald-500/10 border border-emerald-400/30 rounded-xl p-5 text-center">
              <p className="text-sm text-slate-300">Total annual cost of paper logs</p>
              <p className="text-3xl font-extrabold text-white mt-1">
                ${totalAnnualCost.toFixed(0)}
              </p>
              <div className="mt-2 text-emerald-300 font-semibold">
                Switch to SiteSafe and save ${savings.toFixed(0)}/year ({savingsPercent}%)
              </div>
