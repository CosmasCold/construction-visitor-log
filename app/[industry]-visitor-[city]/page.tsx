// app/[industry]-visitor-[city]/page.tsx
...
export async function generateMetadata({
  ...
  const description = `Looking for a ${industryData.name.toLowerCase()} visitor check‑in solution in ${cityData.name}? SiteSafe offers QR check‑in, mandatory safety acknowledgment, and flat $49/mo pricing.`;

  return {
    title,
    description,
    alternates: {
      canonical: `https://sitesafe.thesift.space/${params.industry}-visitor-${params.city}`,
    },
    openGraph: {
      title,
      description,
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <FeatureCard
            icon={QrCode}
            title="QR check‑in"
            desc={`Each ${industryName.toLowerCase()} site gets a unique QR code. Visitors scan and sign in — no app, no clipboard.`}
          />
          <FeatureCard
            icon={ShieldCheck}
            title="Mandatory safety acknowledgment"
            desc={`Every visitor must confirm your safety rules before entering. That's your audit‑proof record for ${cityName} inspectors.`}
          />
          <FeatureCard
            icon={Users}
            title="Real‑time dashboard"
            desc={`See exactly who's on your ${industryName.toLowerCase()} site right now, auto‑refreshed every few seconds.`}
          />
          <FeatureCard
            icon={FileDown}
            title="Audit‑ready exports"
            desc="CSV, Excel, PDF — filtered by date, site, or host. Ready when you need them."
          />
          <FeatureCard
            icon={CheckCircle2}
            title="Flat $49/mo"
          ...
          <FeatureCard
            ...
            desc="Try everything free for two weeks. No credit card, no sales calls."
          />
        </div>
      </div>
