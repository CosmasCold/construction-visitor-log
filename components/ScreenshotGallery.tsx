...
export default function ScreenshotGallery({
  ...
  screenshots: Screenshot[];  // ✅ fixed from Screenshots[] to Screenshot[]
}) {
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-24">
      ...
      {/* Outer container with a distinct background */}
      <div className="bg-slate-950/50 backdrop-blur-md rounded-3xl border border-white/10 shadow-card-raised p-6 sm:p-8">
          ...
          >
            <div className="flex gap-6 w-max px-4 sm:px-0">
              {screenshots.map((shot, idx) => (
                <div
                  key={idx}
                  className="snap-center flex-shrink-0 w-[85vw] max-w-md bg-white/[0.08] backdrop-blur-md rounded-2xl border border-white/10 shadow-card-raised overflow-hidden"
                >
                  <div className="p-2">
                    <Image
                      src={shot.src}
        ...
        <p className="text-xs text-slate-500 text-center mt-4">
          Actual product screenshots. Use arrows or scroll to explore.
        </p>
      </div>
    </div>
  );
